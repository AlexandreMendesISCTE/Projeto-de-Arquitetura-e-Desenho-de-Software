...
const ChatWidget = () => {
   ...
   */
  const processN8NResponse = async (response: N8NResponse) => {
    const { message, action, origin: originData, destination: destinationData, location, suggestions } = response

    // Handle set_route action (both origin and destination provided)
    if (action === 'set_route' && originData && destinationData) {
      // Set origin
      setOrigin({
      ...
      const centerLat = (originData.lat + destinationData.lat) / 2
      const centerLng = (originData.lng + destinationData.lng) / 2
      setCenter([centerLat, centerLng])
      setZoom(9)
      
      addBotMessage(`✅ ${message}\n\n🗺️ Rota definida:\n📍 Origem: ${originData.name}\n🎯 Destino: ${destinationData.name}`)
      return
    }

    // Handle set_origin action
    if (action === 'set_origin' && (location || originData)) {
      const locationToSet = originData || location!
      setOrigin({
        name: locationToSet.name,
        lat: locationToSet.lat,
        lng: locationToSet.lng
      })
      setCenter([locationToSet.lat, locationToSet.lng])
      setZoom(12)
      
      // If there are suggestions (destination), add them to the message
      if (suggestions && suggestions.length > 0) {
        const destSuggestion = suggestions[0]
        setDestination({
          name: destSuggestion.name,
          lat: destSuggestion.lat,
          lng: destSuggestion.lng
        })
        addBotMessage(`✅ ${message}\n\n📍 Origem: ${locationToSet.name}\n🎯 Destino: ${destSuggestion.name}`)
      } else {
        addBotMessage(`✅ Origem definida: ${locationToSet.name}\n\n${message}`)
      }
      return
    }

    // Handle set_destination action
    if (action === 'set_destination' && (location || destinationData)) {
      const locationToSet = destinationData || location!
      setDestination({
        name: locationToSet.name,
        lat: locationToSet.lat,
        lng: locationToSet.lng
      ...
      setCenter([locationToSet.lat, locationToSet.lng])
      setZoom(12)
      addBotMessage(`✅ Destino definido: ${locationToSet.name}\n\n${message}`)
      return
    }

    // Handle add_waypoint action
    if (action === 'add_waypoint' && location) {
      addWaypoint({
        name: location.name,
        lat: location.lat,
        lng: location.lng
      ...
      setZoom(12)
      addBotMessage(`✅ Paragem adicionada: ${location.name}\n\n${message}`)
      return
    }

    // Handle clear_route action
    if (action === 'clear_route') {
      setOrigin(null)
      setDestination(null)
      setWaypoints([])
      addBotMessage(`🗑️ Rota limpa.\n\n${message}`)
      return
    }

    // Default: just show the message
    const lowerResponse = message.toLowerCase()
    if (lowerResponse.includes('origem') || lowerResponse.includes('partida')) {
      setWaitingForInput('origin')
      addBotMessage(message + '\n\n👆 Clique no mapa para definir a origem ou digite o nome do local.')
    } else if (lowerResponse.includes('destino') || lowerResponse.includes('chegada')) {
      setWaitingForInput('destination')
      addBotMessage(message + '\n\n👆 Clique no mapa para definir o destino ou digite o nome do local.')
    } else if (lowerResponse.includes('paragem') || lowerResponse.includes('parada')) {
